...
public class TabView extends View {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		CLog.i(TAG, "dispatchTouchEvent:" + f);
		return f;//false时触发父类的onClick
	}
	...
	public TabView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mBadgeBackgroundColor = a.getColor(R.styleable.TabView_badgeBackgroundColor, mBadgeBackgroundColor);
		a.recycle();
		initText();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (TextUtils.isEmpty(mText)) mText = "djfdjklfjk";
		if (mText == null && (mIconNormal == null || mIconSelected == null)) {
			throw new IllegalArgumentException("必须设置tabText或者tabIconSelected、tabIconNormal两个，或者全部设置");
		}
	...
	public void setIconAlpha(float alpha) {
		if (alpha < 0 || alpha > 1) {
			throw new IllegalArgumentException("alpha must between 0.0 and 1.0");
		}
		mAlpha = alpha;
		invalidateView();
	}
